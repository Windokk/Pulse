...
private:
    ...
    // Stable identifier assigned by ImGui::InsertNotificationWithId(), so a long-lived toast can be
    // looked up and mutated in place across frames (ImGui::FindNotification()) even though the
    // notifications vector is compacted as toasts expire. -1 for toasts inserted the classic way.
    int											id = -1;
...
public:
    ...
    */
     ...
     */
    inline void setProgress(const float fraction)
    {
        this->progressFraction = fraction < 0.0f ? -1.0f : (fraction > 1.0f ? 1.0f : fraction);
    }

    /**
     * @brief Override the auto-dismiss delay (ms) after construction - used to keep a progress toast
     * sticky while work is in flight, then shorten it once the work finishes.
     */
    inline void setDismissTime(const int milliseconds)
    {
        this->dismissTime = milliseconds;
    }
    ...
    /**
     * @brief Assign the stable lookup id (see the id member). Set by ImGui::InsertNotificationWithId().
     */
...
public:
    ...
    */
    ...
    /**
     * @return This toast's stable lookup id, or -1 if it wasn't inserted with one.
     */
    inline int getId()
    {
        return this->id;
...
namespace ImGui
{
    ...
    /**
     * @brief Inserts a notification and stamps it with a stable id, which is returned so the caller
     * can later mutate the same toast in place via FindNotification() (progress bars, live status
     * text). Unlike a raw index into `notifications`, this id stays valid as other toasts expire.
     */
    inline int InsertNotificationWithId(ImGuiToast toast)
    {
        toast.setId(nextNotificationId++);
        notifications.push_back(toast);
        return toast.getId();
    }

    /**
     * @brief Returns the live toast previously inserted with the given id, or nullptr if it has since
     * expired / been dismissed. The pointer is only valid until the next insert/remove.
     */
    inline ImGuiToast* FindNotification(int id)
    {
        if (id < 0)
            return nullptr;
    ...
    /**
     * @brief Removes the toast with the given stable id, if it is still present.
     */
    inline void RemoveNotificationById(int id)
    {
